...
WORKFLOW_DIR = Path.home() / ".trae-cn" / "workflows"
TEMPLATE_DIR = Path.home() / ".trae-cn" / "templates"
...
示例:
  %(prog)s list                          # 列出所有工作流
  %(prog)s run git-commit-summary        # 执行工作流
  %(prog)s run git-commit-summary --var days=7
        '''
    )
